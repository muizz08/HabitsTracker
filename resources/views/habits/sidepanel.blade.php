                    ...
                    <div class="flex h-full flex-col bg-[#F8F8FA] shadow-2xl">
                        ...
                        <div class="flex-1 overflow-y-auto p-5">
                            ...
                            <form x-ref="taskForm" action="{{ route('tasks.store') }}" method="POST" class="space-y-5"
                                @submit="
                                    // 1. Validasi Array Tag melalui Alpine.js sebelum form terkirim
                                    showTagError = !Array.isArray(form.tags) || form.tags.length === 0;

                                    if (showTagError) {
                                        $event.preventDefault(); // Batalkan submit secara paksa jika tag kosong
                                        return;
                                    }

                                    // 2. Jika dalam mode edit, ubah action URL tujuan secara dinamis
                                    if (panelMode === 'edit') {
                                    $refs.taskForm.action = '/tasks/' + panelTaskId;
                                    } else {
                                        $refs.taskForm.action = '{{ route('tasks.store') }}';
                                    }
                                    
                                    // Form akan melakukan submit alami ke Laravel, menjaga validasi 'required' tetap aktif
                                ">
                                ...
                                <div>
                                    ...
                                    <div class="grid grid-cols-2 gap-3 rounded-2xl transition-all duration-200 p-2"
                                        ...
                                        @foreach($tags as $tag)
                                            <label
                                                class="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-3 py-3 cursor-pointer hover:border-indigo-300 transition">
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        ...
                                        <!-- Hidden input -->
                                        <input type="hidden" name="reminder" :value="form.reminder ? 'true' : 'false'">

                                        <!-- Toggle -->
                                        <input type="checkbox" x-model="form.reminder" class="sr-only peer">
                                ...
                                <div>
                                    ...
                                    <textarea name="description" x-model="form.description" rows="5"
                                        placeholder="Menyelesaikan desain landing page sebelum deadline."
                                        class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none resize-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100"></textarea>
                                </div>

                                <div class="border-t border-slate-200 bg-white pt-5">
                                    <div class="grid grid-cols-3 gap-3">
                                        <button @click="openPanel = false" type="button"
                                            class="rounded-xl border border-slate-200 bg-white py-3 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition">
                                            Batal
                                        </button>
                                        <button @click="deleteTask()" type="button"
                                            class="rounded-xl bg-red-500 py-3 text-sm font-semibold text-white hover:bg-red-600 transition">
                                            Hapus
                                        </button>
                                        <button type="submit"
                                            class="rounded-xl bg-indigo-600 py-3 text-sm font-semibold text-white hover:bg-indigo-700 transition">
                                            Simpan
                                        </button>
                                    </div>
                                </div>
